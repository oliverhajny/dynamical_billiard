...
import pytest
import numpy as np
from billiard.physics import advance_shape, advance_with_time_shape
...
def test_advance_simple_right_hit():
    ...
    impact, new_dir, t = advance_shape(p0, d0, shape, return_t=True)
...
def test_advance_return_without_t_flag():
    ...
    d0 = np.array([0.0, 1.0])
    shape = EllipseShape(a, b)
    out = advance_shape(p0, d0, shape, return_t=False)
...
def test_advance_with_time_dt_matches_t_over_speed():
    ...
    shape = EllipseShape(a, b)
    impact_a, new_dir_a, t_geom = advance_shape(p0, d0, shape, return_t=True)
    # z advance_with_time získáme dt = t/speed
    impact_b, new_dir_b, dt, t_echo = advance_with_time_shape(p0, d0, speed, shape, return_t=True)
...
def test_advance_with_time_speed_must_be_positive():
    ...
    with pytest.raises(ValueError):
        _ = advance_with_time_shape(p0, d0, 0.0, shape, return_t=True)
...
def test_start_on_boundary_pure_tangent_raises():
    ...
    shape = EllipseShape(a, b)
    with pytest.raises(ValueError):
        _ = advance_shape(p0, d0, shape, return_t=True)
...
def test_advance_with_time_tangent_raises_too():
    ...
    shape = EllipseShape(a, b)
    with pytest.raises(ValueError):
        _ = advance_with_time_shape(p0, d0, speed=1.0, shape=shape, return_t=True)
